...
def plot_directed_graph(gc, mid, hide_digits=False):  # for motoneurons
    ...
    if not hide_digits:
        for label, color, pos in zip(label_list, color_list, circle_centers):
            plt.text(pos[0] + np.sign(pos[0]) * 0.4, pos[1], label, va='center', ha='center', color=color,
...
def plot_background_and_cells(cell_centers, subset_neurons, background, subplots=False, invert=False):
    ...
    if subplots:
        ...
        if invert:
            ...
            ax1.scatter(512 - cell_centers[subset_neurons, 0], cell_centers[subset_neurons, 1], color='crimson',
                        edgecolor='black', s=100)
            ax2.scatter(512 - cell_centers[subset_neurons, 0], cell_centers[subset_neurons, 1], color='crimson',
                        edgecolor='black', s=100)
        else:
            ...
            ax1.scatter(cell_centers[subset_neurons, 0], cell_centers[subset_neurons, 1], color='crimson',
                        edgecolor='black', s=100)
            ax2.scatter(cell_centers[subset_neurons, 0], cell_centers[subset_neurons, 1], color='crimson',
                        edgecolor='black', s=100)
        ...
        if invert:
            ...
            plt.scatter(512 - cell_centers[subset_neurons, 0], cell_centers[subset_neurons, 1], color='crimson',
                        edgecolor='black', s=100)
        else:
            plt.scatter(cell_centers[:, 0], cell_centers[:, 1], color='silver', edgecolor='black', s=100)
            plt.scatter(cell_centers[subset_neurons, 0], cell_centers[subset_neurons, 1], color='crimson',
                        edgecolor='black', s=100)
        res = fig
    plt.axis('off')
    plt.tight_layout()
...
def plot_gc_matrix_HB(gc_motorneurons, loc, gc_type='dff_8', savepath=None, figname_suffix=None, title=False,
                      save=False):
    ...
    if title:
        ...
        if is_multiv:
            plt.suptitle(f"Fish {fish}, trace {trace} - Multivariate GC n_lags={n_lags}, {n_sig} significant links",
                         size=20)
        else:
            plt.suptitle(f"Fish {fish}, trace {trace} - Bivariate GC n_lags={n_lags}, {n_sig} significant links",
                         size=20)
    ...
    if invert:
        for j in range(len(cell_centers)):
            if next_sub_idx < len(subset_neurons) and j == subset_neurons[next_sub_idx]:
                ...
                color = default_color
            plt.scatter(512 - cell_centers[j, 0], cell_centers[j, 1], color=color, edgecolor='black',
        ...
        for j in range(len(cell_centers)):
            if next_sub_idx < len(subset_neurons) and j == subset_neurons[next_sub_idx]:
                ...
                color = default_color
            plt.scatter(cell_centers[j, 0], cell_centers[j, 1], color=color, edgecolor='black',
                        s=100 * (int(drive[j] * cst) + 1), alpha=0.8)
    plt.axis('off')
...
def plot_background_and_cells_drive(cell_centers, background, drive, cst, color='crimson', subplots=False,
                                    invert=False):
    """ Plot the brain image of the plane and corresponding cells (of a fish-trial pair).
    To make two subplots (for info flow plots) use subplots=True.
    To only draw background without cells, use scatter=False (for emit/receive plots).
...
def plot_info_flow_HB(gc, cell_centers, background, fish, drive_type='sum_out', zoom=True, xlims=[210, 360],
                      ylims=[85, 165], cst=30):
    ...
    fig, (ax1, ax2) = plot_background_and_cells_drive(cell_centers, background, drive, cst, subplots=True,
                                                      invert=invert)
        ...
        for i_to in range(len(cell_centers)):
            if gc[i_from, i_to] > 0:
                ...
                if x_from <= x_to:  # rostral to caudal
                    ax1.arrow(x_from, y_from, x_to - x_from, y_to - y_from, color='crimson', length_includes_head=True,
                              width=width, alpha=0.6)
                if x_from >= x_to:  # caudal to rostral
                    ax2.arrow(x_from, y_from, x_to - x_from, y_to - y_from, color='crimson', length_includes_head=True,
                              width=width, alpha=0.6)
